...
if (isset($_POST['fetchNew']) && $_POST['fetchNew'] == 'true') {
    ...
    $stmt = $pdo->prepare("SELECT ii.*, l.name FROM startups_information ii INNER JOIN login l ON ii.startup_id = l.id WHERE ii.startup_id > :lastId ORDER BY ii.investor_id ASC LIMIT 1");
    $stmt->bindParam(':lastId', $lastId, PDO::PARAM_INT);
    $stmt->execute();
    $profileImagePath = $newContent['image'];
    ...
    $name = $newContent['startup_name'];
    $vision = $newContent['vision'];
    $usp= $newContent['usp'];
    $motivation = $newContent['motivation'];
    $target_market= $newContent['target_market'];
    $Founding_year =$newContent['Founding_year'];
    $customer_num = $newContent['customer_num'];
    $revenue = $newContent['revenue_till_date'];
    
    if ($newContent) {
        $html = "
        <div id='contentWrapper' class='wrapper max-w-2xl mx-auto bg-[#110B11] rounded-xl overflow-hidden shadow-lg' data-last-id='0'>.
            ...
            <div class='max-w-2xl mx-auto bg-[#110B11] rounded-xl overflow-hidden shadow-lg'>.
                <div class='p-0 items-center'>.
                    <div class='bg-gradient-to-r from-gray-500 to-gray-600 text-white p-2 rounded-md mb-4 shadow w-auto max-w-xs mx-auto'>.
                        <h2 class='font-semibold text-center'>{$industry}</h2>.
                    </div>.
                    <div class='bg-gradient-to-r from-gray-500 to-gray-600 text-white p-2 rounded-md mb-4 shadow w-auto max-w-xs mx-auto'>.
                        <p class='text-center'>{$newContent['location']}</p>.
                    </div>.
                </div>.
                <div class='flex justify-center items-center mt-4'>.
                    <div class='w-44 h-26 bg-gray-300 rounded-lg overflow-hidden'>
                        <img src='{$relativePath}' class='w-full h-full object-cover' alt='Company Logo'>
                    </div>
                </div>
                <center>
                    ...
                    <div class='mt-8 sm:mt-12'>
                        <dl class='grid grid-cols-1 gap-4 sm:grid-cols-3'>
                            <div class='flex flex-col rounded-lg bg-gradient-to-r from-gray-500 to-gray-600 px-4 py-8 text-center dark:from-gray-700 dark:to-gray-800'>
                                ...
                                <dd class='text-4xl font-extrabold text-blue-600 md:text-5xl dark:text-blue-50'>{$Target_market}</dd>
                            </div>
                            <div class='flex flex-col rounded-lg bg-gradient-to-r from-gray-500 to-gray-600 px-4 py-8 text-center dark:from-gray-700 dark:to-gray-800'>
                                <dt class='order-last text-lg font-medium text-gray-500 dark:text-white/75'>
                                 Customers (100s)
                                </dt>
                                <dd class='text-4xl font-extrabold text-blue-600 md:text-5xl dark:text-blue-50'>{$customer_num}</dd>
                            </div>
                            <div class='flex flex-col rounded-lg bg-gradient-to-r from-gray-500 to-gray-600 px-4 py-8 text-center dark:from-gray-700 dark:to-gray-800'>
                                <dt class='order-last text-lg font-medium text-gray-500 dark:text-white/75'>
                                  Revenue (Lakhs)
                                </dt>
                                <dd class='text-4xl font-extrabold text-blue-600 md:text-5xl dark:text-blue-50'>{$revenue}</dd>
                            </div>
                        </dl>
                    </div>
                </section>
...
document.addEventListener("DOMContentLoaded", function() {
    fetchNewContent(true); // Initial load with true to indicate initial fetch
...
function swipeCard(direction) {
    ...
    setTimeout(function() {
        fetchNewContent();
        wrapper.classList.remove('swipe-up', 'swipe-down');
    }, 3000); // Ensure this matches your CSS animation duration
}
function fetchNewContent() {
    var lastId = document.querySelector('.wrapper').getAttribute('data-last-id');
    var formData = new FormData();
    formData.append('fetchNew', 'true');
    ...
    .then(data => {
        if (data.success) {
            // Completely replace the innerHTML of the wrapper with new content
            document.querySelector('.wrapper').innerHTML = data.html;
            // Update the last ID attribute to the new last ID
            document.querySelector('.wrapper').setAttribute('data-last-id', data.lastId);
        } else {
            // Handle the case where no more new content is available
